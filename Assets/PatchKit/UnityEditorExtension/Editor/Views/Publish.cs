...
public class Publish : IView
{
    ...
    public Publish(string buildDir, Api.Models.Main.App selectedApp)
    {
        _buildDir = buildDir;
    ...
    public void Show()
    {
        ...
        EditorGUILayout.BeginHorizontal();
        {
            ...
            _autoPublishAfterUpload =
                EditorGUILayout.Toggle(_autoPublishAfterUpload);
        }
        EditorGUILayout.EndHorizontal();
        ...
        if (CanBuild())
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    if (!CharactersValidator.Validate(_changelog) ||
                        !CharactersValidator.Validate(_label))
                    {
                        if (EditorUtility.DisplayDialog(
                            "Warning",
                            "Use only English characters and ':', '_' or '-' in label and changelog input.\n \n" +
                        ...
                        {
                            Application.OpenURL(
                                "https://panel.patchkit.net/apps/" +
                                _selectedApp.Id); //production
