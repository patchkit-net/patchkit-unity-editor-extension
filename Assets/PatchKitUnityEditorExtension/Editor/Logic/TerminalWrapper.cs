    ...
    public class TerminalWrapper
    {
        ...
        public static string ExecutableByType(Type type)
        {
            switch (type)
            {
                ...
                default:
                    BuildAndPublish.messagesView.AddMessage("Unknown or unsupported terminal.",
        ...
        public void Launch(string argumentsString)
        {
            ...
            BuildAndPublish.messagesView.AddMessage(string.Format("Launching {0} {1}", filename, args),
                UnityEditor.MessageType.Info);
            var process = Process.Start(processInfo);

            if (process.HasExited && process.ExitCode != 0)
            {
                BuildAndPublish.messagesView.AddMessage(string.Format("Non zero ({0}) return code.", process.ExitCode),
                    UnityEditor.MessageType.Warning);
                throw new Exception(string.Format("Non zero ({0}) return code.", process.ExitCode));
            }

            new Thread(() => { process.WaitForExit(); });
        }
    }
}